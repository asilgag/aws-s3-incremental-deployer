...
class AwsS3IncrementalDeployer
{
    ...
    public function deploy(
        ...
        if ($deployedSiteChecksums) {
            ...
            $this->deployIncremental(
                $localSiteChecksums,
                $deployedSiteChecksums,
                $alwaysIncremental
            );
        } else {
     ...
     */
    protected function deployIncremental(array $newChecksums, array $oldChecksums, bool $alwaysIncremental): void
    {
    ...
    protected function deployFull(bool $alwaysIncremental): void
    {
        ...
        $this->logger->info(
            'Syncing everything and deleting stale content from S3 bucket...'
        );
        $this->s3sync(
            $this->siteDir,
            $this->bucketRootUri,
